...
def create_password_protected_zip(file_name, password):
    try:
        pyminizip.compress(file_name, None, f"locked.zip", password, 5)
...
def extract_password_protected_zip(zip_file_name, password):
    try:
        ...
        print(
            f"Error: The ZIP file is corrupted or not in the correct format: {str(e)}"
        )
...
def encode_to_frames(input_file):
    ...
    video = cv2.VideoWriter(
        f"{file_name}.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 30, (width, height)
    )
        ...
        for a in range(len(density_binary)):
            if density_binary[a] == "1":
                ImageDraw.Draw(img).rectangle(
                    (w_pix, h_pix, w_pix + (width / 8) - 1, h_pix + (height / 2) - 1),
                    fill="white",
                    outline=None,
                    width=1,
                )
    ...
    for a in range(len(str(density))):
        density_binary = "".join(
            format(byte, "08b") for byte in str(density)[a].encode("utf-8")
        )
    ...
    with open(input_file, "rb") as f:
        ...
        with tqdm(total=byte_count, unit="byte", unit_scale=True) as pbar:
            while byte := f.read(1):
                binary = "{0:08b}".format(int(hex(byte[0])[2:], 16))
                for a in range(len(binary)):
                    if binary[a] == "1":
                        ImageDraw.Draw(img).rectangle(
                            (w_pix, h_pix, w_pix + density - 1, h_pix + density - 1),
                            fill="white",
                            outline=None,
                            width=1,
                        )
    ...
    for a in range(2):
        for b in range(8):
            ...
            if color[0] > 128:
                ...
                binary += "0"
    density = int(
        bytes(int(binary[i : i + 8], 2) for i in range(0, len(binary), 8)).decode(
            "utf-8"
        )
    )
    binary = ""
...
def main():
    ...
    if user_choice == "1":
        ...
        input_file = error_handling(
            "Enter the file to decode (i.e, encoded_file.mp4): ", os.path.isfile
        )
